...
void run_tests(object me) {
    ...
    assert_equal("鹿港", lukang["name"], "鹿港名稱欄位");
    assert_equal(15000, lukang["population"], "鹿港人口值");
    assert_equal(80, lukang["cohesion"], "鹿港凝聚力值");
    assert_equal(1, member_array("木雕", lukang["industry"]) != -1, "鹿港特產包含木雕");

    // 載入台南
    mapping tainan = set_d->load_settlement("tainan");
    assert_equal(1, mapp(tainan), "台南聚落資料應為 Mapping");
    assert_equal("台南", tainan["name"], "台南名稱欄位");
    assert_equal(85000, tainan["population"], "台南人口值");
